...
- **Claude Code** — full integration via the official hook protocol. See current state, get a notch-side approval panel for every `PermissionRequest`, auto-deny after 30 s of no decision.
...
- **Buddy** — pick one of 9 hand-drawn pixel companions. Lives in your menu bar.
...
3. Use Claude Code normally. The notch icon turns:
   - 🔵 cyan — processing
   - 🟠 orange — needs your approval (with the panel + 30 s timer)
   - 🟢 green — ready / done

Gemini CLI: just use it. If your data dir is one of `~/.gemini`, `~/.config/gemini`, `~/.config/google/gemini`, MioMini will pick it up. Otherwise set `MIOMINI_GEMINI_DIR=/your/path` before launching the app.
...
- [ ] Real notch floating panel (currently uses standard `MenuBarExtra` dropdown)
...
The notch-as-status-bar idea is borrowed from [MioIsland](https://github.com/MioMioOS/MioIsland) by xmqywx. MioMini intentionally trades MioIsland's full feature set (iPhone sync, plugin marketplace, terminal jumping) for a smaller, more security-conscious surface area.
